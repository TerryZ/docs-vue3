...
export function useSelectPageHandle (dataList = nbaTeams) {
  function dataListHandle (data) {
    ...
    const list = search
      ? dataList.filter(val => val.name.toLocaleLowerCase().includes(search.toLocaleLowerCase()))
      : dataList
  ...
  function fetchData (data, callback) {
    ...
    setTimeout(() => callback(result.list, result.count), 300)
  }
  /**
